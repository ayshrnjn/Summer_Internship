...
function EditClient() {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      try {
        ...
        setWebsiteCredentials(res.data.websiteCredentials.length ? res.data.websiteCredentials : [{ username: '', password: '', url: '' }]);
        setDomains(res.data.domains.length ? res.data.domains : [{ domainName: '', registrar: '', expiryDate: '', renewalCharge: '' }]);
        setEmailCredentials(res.data.emailCredentials.length ? res.data.emailCredentials : [{ email: '', password: '', provider: '' }]);
  ...
  return (
    <div className="centered-container">
      <motion.div className="auth-card" style={{ maxWidth: 600 }} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}>
        <h2>Edit Client</h2>
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="primary-btn">Update Client</button>
        </form>
      </motion.div>
    </div>
  );
